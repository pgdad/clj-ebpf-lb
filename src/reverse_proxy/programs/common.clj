...
(defn stx-b
  ...
  [dst off src]
  (dsl/stx :b dst off src))

(defn stx-h
  "Store half-word to memory."
  [dst off src]
  (dsl/stx :h dst off src))

(defn stx-w
  "Store word to memory."
  [dst off src]
  (dsl/stx :w dst off src))

(defn stx-dw
  "Store double-word to memory."
  [dst off src]
  (dsl/stx :dw dst off src))
...
(defn build-bounds-check
  "Generate instructions to check if accessing [data + offset, data + offset + size)
   is within packet bounds. Jumps to fail-label if out of bounds.

   Assumes: r6 = data start, r7 = data end
   Uses: r8 as scratch"
  [offset size fail-label]
  [(mov-reg :r8 :r6)           ;; r8 = data start
   (add-imm :r8 (+ offset size)) ;; r8 = data + offset + size
   (dsl/jmp-reg :jgt :r8 :r7 fail-label)]) ;; if r8 > data_end, jump to fail

(defn build-parse-eth
  "Parse Ethernet header and check for IPv4.

   Assumes: r6 = data, r7 = data_end
   After: jumps to pass-label if not IPv4
   Uses: r8 as scratch"
  [pass-label]
  (concat
    ;; Bounds check for Ethernet header
    (build-bounds-check 0 ETH-HLEN pass-label)
    ;; Load ethertype (offset 12 in eth header)
    [(ldx-h :r8 :r6 12)
     ;; Check if IPv4 (0x0800)
     ;; Note: ldx loads in host byte order, but ethertype is network order
     ;; On little-endian, we need to compare with swapped value
     ;; For simplicity, assume network byte order comparison
     (dsl/jmp-imm :jne :r8 ETH-P-IP pass-label)]))
...
(defn build-parse-ip
  ...
  (let [ip-off ETH-HLEN]
    (concat
      ...
      (build-bounds-check ip-off IP-HLEN-MIN pass-label)
      ...
      [(ldx-b :r8 :r6 ip-off)
       ...
       ;; Verify IHL >= 20
       (dsl/jmp-imm :jlt :r8 IP-HLEN-MIN pass-label)
       ...
       (ldx-w :r8 :r10 -4)
       (dsl/jmp-imm :jeq :r8 IPPROTO-TCP :continue-l4)
       (dsl/jmp-imm :jeq :r8 IPPROTO-UDP :continue-l4)
       (dsl/ja pass-label)
...
(defn build-parse-l4
  "Parse TCP/UDP header to extract ports.
   ...
   Assumes: stack[-16] = IP header length
   Stores on stack:
     stack[-20] = src port
     stack[-24] = dst port
   Uses: r8, r9"
  ...
  [(ldx-w :r8 :r10 -16)        ;; r8 = IP header length
   ...
   (add-reg :r8 :r9)
   (add-imm :r8 4)
   (dsl/jmp-reg :jgt :r8 :r7 pass-label)

   ;; Calculate L4 header address: data + L4_offset
   (mov-reg :r8 :r6)
   (add-reg :r8 :r9)

   ;; Load src port (offset 0)
   (ldx-h :r9 :r8 0)
   (stx-h :r10 -20 :r9)

   ;; Load dst port (offset 2)
   (ldx-h :r9 :r8 2)
   (stx-h :r10 -24 :r9)])
...
(defn flatten-instructions
  "Flatten nested instruction vectors and handle labels.
...
(defn assemble-program
  ...
  (let [instructions (apply flatten-instructions fragments)]
    ;; TODO: Label resolution pass
    ;; For now, return raw instructions
    ;; clj-ebpf should handle label resolution in its program loading
    (dsl/program instructions)))
